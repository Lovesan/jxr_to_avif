...
int main(int argc, char *argv[])
{
    try
    {
        ...
        int hr = jxr_init_loader_thread();

        if (hr < 0)
        {
            auto errorDesc = jxr_get_error_description(hr);
            std::cerr << "Failed to initialize JXR loader thread: " << errorDesc << "\n";
        ...
        hr = jxr_load_data(inputFile, &jxr_data);

        if (hr < 0)
        {
            auto errorDesc = jxr_get_error_description(hr);
            std::cerr << "Failed to get image data: " << errorDesc << "\n";
            jxr_free_error_description(errorDesc);
            return hr;
        }
        ...
        if (converted == nullptr)
        {
            ...
            jxr_free_data(&jxr_data);
            jxr_deinit_loader_thread();
            return 1;
        }

        uint32_t numThreads = jxr_get_number_of_processors();
        ...
        uint16_t maxCLL, maxPALL;
        {
            ...
            for (uint32_t i = 0; i < convThreads; i++) {
                ...
                {
                    std::cerr << "Failed to allocate thread data\n";
                    return 1;
                }
